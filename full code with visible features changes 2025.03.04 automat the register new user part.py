...
def recognize_face():
    ...
    detection_count = 0  # Counter for consecutive detections of the same person
    greeting_triggered = False  # Flag for initial greeting
    no_face_count = 0
    last_state = None
    unknown_count = 0
    ...
    while True:
        ...
        if embeddings_batch:
            ...
            if highest_confidence > facenet_threshold:
                ...
                # Check if the same person is detected
                if best_match == last_recognized_name:
                    detection_count += 1
                else:
                    # New person detected, reset counter and state
                    last_recognized_name = best_match
                    detection_count = 1  # Start counting for the new person
                    greeting_triggered = False
                    no_face_count = 0
                    last_state = None
                
                # Trigger initial greeting after 5 consecutive detections
            ...
            if last_recognized_name is not None and greeting_triggered:
                no_face_count += 1
                if no_face_count == 5 and last_state == 'recognized':  # Trigger exactly at 5
                    ...
                    if tts_engine:
                        ...
                        tts_engine.runAndWait()
                    last_state = 'no_face'
            elif last_recognized_name is not None:
                # Reset only if no greeting has been triggered yet
                detection_count = 0
                greeting_triggered = False
            unknown_count = 0
