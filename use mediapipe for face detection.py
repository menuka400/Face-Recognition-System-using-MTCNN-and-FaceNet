...
# Path to the BlazeFace model
BLAZEFACE_MODEL_PATH = r"C:\Users\menuk\Desktop\Face-Recognition-System-using-BlazeFace-and-FaceNet\blaze_face_short_range.tflite"

# === MediaPipe Initialization ===
def initialize_face_detector():
    """Initialize MediaPipe face detector with the BlazeFace short range model"""
    try:
        ...
        # Create the face detector with the image mode (we'll process frames individually)
        options = FaceDetectorOptions(
            base_options=BaseOptions(model_asset_path=BLAZEFACE_MODEL_PATH),
        ...
        print(f"Error initializing MediaPipe face detector: {str(e)}")
        return None
...
# === Face Detection and Feature Extraction (Single Face Only) using MediaPipe BlazeFace ===
def detect_and_extract_features(frame, face_detector, facenet_model):
    if frame is None:
    ...
    if face_roi.size == 0:
        cv2.putText(frame_with_box, "Invalid face region", (10, 30), 
                    cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 0, 255), 2)
        return None, None, None, frame_with_box
    ...
    cv2.putText(frame_with_box, f"Face detected ({score:.2f})", (10, 30), 
...
def register_user(user_name):
    ...
    if face_detector is None or facenet_model is None:
        return False
    ...
    try:
        while True:
            ...
            facenet_embedding, face_roi, bbox, frame_with_box = detect_and_extract_features(
                frame, face_detector, facenet_model
            )
        ...
        face_detector.close()  # Properly close the MediaPipe model
...
def run_diagnostics():
    ...
    try:
        ...
        if detector is not None:
            print(f"✓ MediaPipe BlazeFace detector loaded successfully from {BLAZEFACE_MODEL_PATH}")
            detector.close()  # Properly close the model after testing
        else:
            print("✗ MediaPipe BlazeFace detector failed to load")
            return False
    except Exception as e:
        print(f"✗ MediaPipe BlazeFace detector error: {str(e)}")
        return False
