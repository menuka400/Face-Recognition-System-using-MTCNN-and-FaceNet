...
from mtcnn.mtcnn import MTCNN  # Explicitly import MTCNN class
import logging
...
def initialize_detector():
    ...
    try:
        detector = MTCNN()  # Default constructor, no custom params
        return detector
    except Exception as e:
...
def get_face_embedding(model, face_img):
    ...
    try:
        ...
        embedding = embeddings[0] / np.linalg.norm(embeddings[0])  # Normalize to unit length
        return embedding
    except Exception as e:
...
def detect_and_extract_features(frame, detector, facenet_model):
    ...
    if face['confidence'] < 0.9:  # Stricter threshold
        return None, None, None, frame_with_box
...
def recognize_face():
    ...
    facenet_threshold = 0.75  # Optimized threshold
    frame_count = 0
